...
export default function CartaoProduto(props: CartaoProdutoProps) {
  ...
  return (
    <div className="flex flex-col w-72 bg-teal-950">
      <div className="relative w-72 h-52">
        <Image src={imagem} alt={nome} fill className="object-cover" />
      </div>
      <div className="flex-1 flex flex-col gap-4 p-5">
        <h2 className="text-xl font-bold">{nome}</h2>
        <p className="flex-1  text-base text-zinc-300">{descricao}</p>
        <div className="flex justify-between items-center">
          ...
          <button
            ...
            className="border rounded-full px-5 py-1 text-sm font-bold hover:bg-teal-700"
          >
            Adicionar
          </button>
        </div>
